...
function loadJSON(callback) {   
  ...
  xobj.open('GET', 'data/materialData.json', true);
  ...
  // Parse JSON string into object
    ...
    data["materials"].forEach(function(item){
    	...
    	if ( item.link != undefined ) {
    		materialTableHTML += "<a class='"+item.class+"' href='"+item.link+"'>";
    	...
    	materialTableHTML += "<a class='"+item.class+"' href='#top' onclick='setDensity(\""+matDensity.to("kg/m3").toPrec(0.01).toString()+"\")'>";
    	materialTableHTML += matDensity.toString("kg/m3");
    	materialTableHTML += "</a></td><td>";
    	materialTableHTML += "<a class='"+item.class+"' href='#top' onclick='setDensity(\""+matDensity.to("lb/ft3").toPrec(0.01).toString()+"\")'>";
